...
from apps.incident.handlers.commands.incident import handle_choose_incident
...
def test_handle_choose_incident_announces_what_was_drawn():
    ...
    with mock.patch("apps.incident.handlers.commands.incident.random.choices", return_value=[PloughHoard]):
...
def test_handle_choose_incident_on_a_quiet_month():
    ...
    with mock.patch("apps.incident.handlers.commands.incident.random.choices", return_value=[None]):
        result = handle_choose_incident(context=ChooseIncident(faction=faction, month=3))
...
def test_handle_choose_incident_draws_only_from_what_is_possible():
    ...
    with mock.patch("apps.incident.handlers.commands.incident.random.choices", return_value=[None]) as mocked_draw:
