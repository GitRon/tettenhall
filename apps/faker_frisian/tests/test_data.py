...
from apps.faker_frisian.data import BYNAMES, GIVEN_NAMES, PLACE_ELEMENTS, PLACE_GENERICS

ORTHOGRAPHY = re.compile(r"[A-Za-z]+")

# What the draw in "unique_name" needs under it. Twenty living warriors out of two hundred names hold a
# duplicate between them 63% of the time - the birthday problem - and the five redraws are what carry
# that. Below this the roster turns into a byname list.
MINIMUM_POOL = 200
...
def test_given_names_are_a_deep_enough_pool():
    """
    The reason this pool is composed rather than collected. The attested Old Frisian stock is forty-odd
    names, which the retry cannot work against.
    """
    result = len(GIVEN_NAMES)

    assert result >= MINIMUM_POOL


def test_given_names_obey_the_orthography():
    """
    No length marks, thorn and eth resolved to "th", no Latinised endings - which together mean every
    name is plain ASCII letters.
    """
