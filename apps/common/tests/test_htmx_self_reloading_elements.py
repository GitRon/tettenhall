...
def _self_reloading_elements_without_a_swap(*, file: Path) -> list[str]:
    ...
    for match in OPENING_ELEMENT_PATTERN.finditer(content):
        ...
        if "hx-get" not in element or not SELF_RELOADING_TRIGGER_PATTERN.search(element):
            continue
        if "hx-swap" in element:
            continue

        violations.append(f"{file.name}:{content.count('\n', 0, match.start()) + 1}")
...
def test_self_reloading_elements_declare_how_they_are_swapped():
    ...
    """
    violations = [
        violation for file in _template_files() for violation in _self_reloading_elements_without_a_swap(file=file)
    ]
