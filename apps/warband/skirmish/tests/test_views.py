...
def test_skirmish_list_view_lists_the_open_fights_oldest_first(logged_in_client, current_savegame):
    ...
    SkirmishFactory(
        attacking_faction=current_savegame.player_faction, victorious_faction=current_savegame.player_faction
    )
...
@pytest.mark.django_db
def test_skirmish_list_view_lists_the_decided_fights_newest_first(logged_in_client, current_savegame):
    first_fight = SkirmishFactory(
        attacking_faction=current_savegame.player_faction, victorious_faction=current_savegame.player_faction
    )
    second_fight = SkirmishFactory(
        attacking_faction=current_savegame.player_faction, victorious_faction=current_savegame.player_faction
    )
    ...
    assert response.status_code == 200
    assert list(response.context["decided_skirmish_list"]) == [second_fight, first_fight]


@pytest.mark.django_db
def test_skirmish_list_view_without_any_fights_leaves_both_lists_empty(logged_in_client, current_savegame):
    response = logged_in_client.get(reverse("warband:skirmish-list-view"))

    assert response.status_code == 200
    assert list(response.context["open_skirmish_list"]) == []
    assert list(response.context["decided_skirmish_list"]) == []


@pytest.mark.django_db
def test_skirmish_list_view_with_only_decided_fights_leaves_the_open_list_empty(logged_in_client, current_savegame):
    decided_fight = SkirmishFactory(
        attacking_faction=current_savegame.player_faction, victorious_faction=current_savegame.player_faction
    )
    ...
    assert list(response.context["open_skirmish_list"]) == []
    assert list(response.context["decided_skirmish_list"]) == [decided_fight]


@pytest.mark.django_db
def test_skirmish_list_view_hides_a_skirmish_of_another_savegame(logged_in_client, current_savegame):
    fight_in_another_savegame = SkirmishFactory()
    SkirmishFactory(
        attacking_faction=fight_in_another_savegame.attacking_faction,
        victorious_faction=fight_in_another_savegame.attacking_faction,
    )
...
def test_skirmish_fight_view_cannot_show_a_skirmish_of_another_savegame(logged_in_client, current_savegame):
    ...
    response = logged_in_client.get(reverse("warband:skirmish-fight-view", kwargs={"pk": other_skirmish.pk}))
...
def test_skirmish_fight_view_sends_the_player_back_with_another_skirmish_running(logged_in_client, current_savegame):
    ...
    assert response.url == reverse("warband:skirmish-list-view")
...
def test_skirmish_finish_round_view_cannot_finish_a_round_of_another_savegame(logged_in_client, current_savegame):
    ...
    response = logged_in_client.post(
        reverse("warband:skirmish-finish-round-view", kwargs={"pk": other_skirmish.pk}),
        data={
...
def test_skirmish_round_update_htmx_view_shows_the_skirmish(logged_in_client, current_savegame):
    ...
    response = logged_in_client.get(reverse("warband:skirmish-round-update-htmx", kwargs={"pk": skirmish.pk}))
...
def test_skirmish_round_update_htmx_view_cannot_show_a_skirmish_of_another_savegame(logged_in_client, current_savegame):
    ...
    response = logged_in_client.get(reverse("warband:skirmish-round-update-htmx", kwargs={"pk": other_skirmish.pk}))
...
def test_skirmish_fight_button_update_htmx_view_shows_the_skirmish(logged_in_client, current_savegame):
    ...
    response = logged_in_client.get(reverse("warband:skirmish-fight-button-update-htmx", kwargs={"pk": skirmish.pk}))
...
def test_skirmish_fight_button_update_htmx_view_cannot_show_a_skirmish_of_another_savegame(
    ...
    response = logged_in_client.get(
        reverse("warband:skirmish-fight-button-update-htmx", kwargs={"pk": other_skirmish.pk})
    )
...
def test_battle_history_update_htmx_view_hides_history_of_another_savegame(logged_in_client, current_savegame):
    ...
    response = logged_in_client.get(
        reverse("warband:battle-history-update-htmx", kwargs={"skirmish_id": other_skirmish.id})
    )
