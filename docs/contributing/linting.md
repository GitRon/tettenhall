...
- **`apps.faker_old_english` must not import `apps.common` either.** It is a name provider and nothing
  else, which is what keeps it liftable into a library of its own.
