...
`Culture`, `ItemType`, `QuestType` and `InjuryType` are lookup tables, not test data. They ship as
fixtures (`apps/warband/fixtures/culture.json`, `apps/warband/fixtures/itemtype.json`,
`apps/warband/fixtures/questtype.json`, `apps/warband/fixtures/injurytype.json` — Django discovers fixtures at `<app>/fixtures/` only, so they sit
...
cultures, item types, quest types or injury types**, and don't build look-alikes: a test that creates its own
